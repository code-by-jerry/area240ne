...
const DEFAULT_SLIDE: HeroSlide = {
    id: 0,
    image_path: 'https://ik.imagekit.io/area24onestorage/area24one%20layout%20images/slide%20default.jpg',
...
const SlideContent = memo(({ slide, index, prefersReducedMotion }: { slide: HeroSlide; index: number; prefersReducedMotion: boolean }) => {
    ...
    return (
        <div className="relative z-10 flex h-full items-center px-4 md:px-12 lg:px-20 justify-center py-8 md:py-0">
            <div className={`flex w-full flex-col gap-4 md:gap-8 lg:flex-row lg:items-center justify-center ${isFirstSlide ? 'lg:gap-24' : 'max-w-5xl'}`}>
                {/* Text Content */}
                <div className={`text-center ${isFirstSlide ? 'max-w-xl lg:text-left' : 'mx-auto max-w-3xl space-y-2 md:space-y-6'}`}>
                    {slide.tag && (
                        <motion.div
                            {...animationProps}
                    ...
                    {slide.title && (
                        <motion.h1 
                            ...
                            className={`font-black tracking-tighter text-white drop-shadow-[0_2px_8px_rgba(0,0,0,0.6)] leading-[1.1] ${
                                isFirstSlide 
                                ? 'mb-2 md:mb-4 text-xl md:text-5xl lg:text-6xl' 
                    ...
                    {slide.description && (
                        <motion.p 
                            ...
                            className={`text-slate-100 font-medium drop-shadow-[0_1px_3px_rgba(0,0,0,0.4)] ${
                                isFirstSlide 
                                ? 'mb-4 md:mb-8 text-[11px] md:text-lg lg:text-xl opacity-90' 
                                : 'mx-auto max-w-2xl text-[12px] md:text-xl opacity-90'
                            }`}
                        >
                            {slide.description}
                        </motion.p>
                    ...
                    {slide.button_text && slide.button_link && (
                        ...
                        >
                            ...
                            {isFirstSlide && (
                                <Link
                                    href="/estimate"
                                    className="group relative inline-flex items-center justify-center overflow-hidden rounded-full border border-white/30 bg-white/10 px-4 py-1.5 md:px-8 md:py-3 text-[11px] md:text-base font-bold text-white backdrop-blur-md transition-all hover:bg-white/20 hover:scale-105 active:scale-95"
                                >
                                    <span className="relative z-10">Estimate Cost</span>
                                </Link>
                            )}
                        </motion.div>
                    )}
                ...
                {isFirstSlide && (
                    <motion.div
                        initial={prefersReducedMotion ? false : { x: 100, opacity: 0, rotate: 5 }}
                        animate={prefersReducedMotion ? false : { x: 0, opacity: 1, rotate: 0 }}
...
export const HeroCarousel = memo(function HeroCarousel({ slides }: HeroCarouselProps) {
    const slidesWithDefault = useMemo(() => {
        ...
        const sortedDynamicSlides = [...validSlides].sort((a, b) => (a.order || 0) - (b.order || 0));
    ...
    useEffect(() => {
        ...
        const preloadImages = () => {
            slidesWithDefault.forEach((slide, index) => {
                const img = new Image();
    ...
    const startTimer = useCallback(() => {
        ...
        timerRef.current = setInterval(() => {
            setCurrentIndex((prev) => (prev + 1) % (slidesWithDefault?.length || 1));
        }, 8000); 
    ...
    const prevSlide = useCallback(() => {
        if (!slidesWithDefault) return;
        setCurrentIndex((prev) => (prev - 1 + slidesWithDefault.length) % slidesWithDefault.length);
        resetTimer();
    }, [slidesWithDefault, resetTimer]);
    ...
    return (
        <div 
            className="relative h-[320px] sm:h-[450px] md:h-screen w-full overflow-hidden bg-brand-dark pt-[88px] md:pt-[80px]"
            onMouseEnter={() => setIsPaused(true)}
            onMouseLeave={() => setIsPaused(false)}
        >
            {/* Progress Bar */}
            <div className="absolute top-0 left-0 z-50 w-full h-1 bg-white/10">
                ...
                >
                    ...
                    >
                        ...
                        <div 
                            ...
                            style={{
                                backgroundImage: `url(${currentSlide.image_path})`,
                                backgroundSize: currentIndex === 0 ? 'cover' : 'contain',
                                backgroundPosition: 'center',
                    ...
                    {/* Content Layer */}
                    <SlideContent slide={currentSlide} index={currentIndex} prefersReducedMotion={!!prefersReducedMotion} />
                </motion.div>
            </AnimatePresence>
            ...
            {slidesWithDefault.length > 1 && (
                <>
                    <button
                        onClick={prevSlide}
                        className="group absolute left-2 top-1/2 z-20 -translate-y-1/2 rounded-full border border-white/10 bg-black/30 p-2 text-white backdrop-blur-md transition-colors hover:bg-black/50 md:left-10 md:p-3"
                    ...
                    <button
                        onClick={nextSlide}
                        className="group absolute right-2 top-1/2 z-20 -translate-y-1/2 rounded-full border border-white/10 bg-black/30 p-2 text-white backdrop-blur-md transition-colors hover:bg-black/50 md:right-10 md:p-3"
                    ...
                    <div className="absolute bottom-3 left-1/2 z-20 flex -translate-x-1/2 gap-1 md:bottom-10 md:gap-2">
                        {slidesWithDefault.map((_, index) => (
                            <button
                                ...
                                onMouseEnter={() => { setCurrentIndex(index); resetTimer(); }}
                                className={`h-1 rounded-full transition-all duration-300 ${
                                    index === currentIndex ? 'w-4 bg-white' : 'w-1 bg-white/40 hover:bg-white/70'
            ...
            {currentIndex !== 0 && (
                <div className="absolute bottom-6 right-6 z-30 md:bottom-10 md:right-10">
                    <ChatWidget />
                </div>
            )}
        </div>
